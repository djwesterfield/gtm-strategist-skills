...
| `/market cro <url>` | Conversion rate optimization expert analysis | CRO-ANALYSIS.md |
| `/market copywriting <url>` | Dedicated copywriting audit and rewrites | COPYWRITING-RECOMMENDATIONS.md |
| `/market sea <goal>` | Paid search strategy and funnel planning | PAID-SEARCH-PLAN.md |
| `/market strategy <context>` | High-level GTM strategy and positioning plan | MARKET-STRATEGY-ROADMAP.md |
...
The new domain-specialist commands map to existing skills and agent templates: 
- `/market seo` → `market-technical` & `running-marketing` SEO blocks
- `/market cro` → `market-conversion` + funnel optimization from `gtm-foundations`
- `/market copywriting` → `market-content` + `crafting-positioning` content strategy
- `/market sea` → `market-ads` + paid channel planning from `building-communication-engine`
- `/market strategy` → `market-strategy` + GTM strategy phases (foundations through launch)
